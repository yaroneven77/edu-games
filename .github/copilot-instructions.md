...
- Body and Wearables has five dropdown choices: picture exploration, build-and-reveal, Beginner, Intermediate, Advanced. The first two offer all 40 characters through `picture-game.js`, using the original Beacon/Pulse PNGs and the 38 approved Stage 1 pictures; the existing levels retain their separate 40-character engine. Keep picture DOM IDs prefixed with `picture-` and retain independent progress for each picture character and mode until reset or reload.
...
- In picture exploration, use the approved high-contrast location outline: an 8px dark outer stroke (`#101827`) and 4px bright yellow inner stroke (`#fff044`), no fill, and non-scaling strokes. Expand each original ellipse radius by 10 drawing units with a minimum of 26, retaining its center and rotation; do not change stored location metadata. Apply this to exploration's vector fallback too. Do not add dimming, pulsing or arrows to the main game. Keep build-and-reveal and the original three levels' highlighting unchanged. The three-option comparison remains under `sandbox/picture-highlight-demo/`.
...
- Both picture games offer 40 characters, ten in each of Anime, Superhero, Cartoon and Manga. The user approved adding the 38 original PNGs from `Chararters/38-cartoon-characters-stage1` alongside Beacon/Pulse, not the rejected generated redraws. Keep both style and character selectors, remembering each style's selected picture character independently of the original levels. Use each character's exact 26 body concepts, fixed 12-item inventory, vector artwork and separately authored original-image coordinates. Returning to the original levels restores the previous collection-style selection.
...
- Use production-local `picture-game.js`, `beacon-art.js`, `pulse-art.js`, the `picture-regions-*.js` maps, `picture-wardrobe.js`, `picture-collection.js` and `picture-art/*.png`, with the existing vocabulary helpers. Display original PNGs unchanged, with lazy per-character loading; never load `Chararters`, rejected asset packages or sandbox files at runtime. Keep the standalone sandbox demo independent. If a reference image fails, show an explicit notice and use that character's vector artwork and coordinates without affecting other characters.
