...
- Award a bonus after every three newly learned non-bonus concepts in a row. Duplicate words/aliases and accepting a bonus word do not advance the next streak. Keep bonus-word IDs for the whole round so accepting an earlier bonus later still does not advance it. Nonempty rejected answers, including known unavailable words, reset the streak; empty input, hints and level changes do not. Reset the streak after awarding a bonus and clear all bonus state on a new round.
...
- New Character reveals the **completed current character**, not a gallery of eight pictures, for **3 seconds**, then randomly selects the other character in the chosen category and starts on an empty canvas. Include the current round's outfit and five selected accessories using the independent layer renderer. This departing-round preview is the sole intentional exception to earned-only visibility; never change learned IDs, clue completion or credit during it. Keep its explanatory caption outside the artwork so it cannot hide the character. Scroll the character into view, lock gameplay/clue/category controls during the preview, prevent duplicate transitions and restore controls afterward. A category reset cancels pending transition timers and starts a fresh round.
